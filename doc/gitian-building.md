...
*Setup instructions for a Gitian build of Alterdot using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Alterdot
...
- [Building Alterdot](#building-alterdot)
...
Clone the git repositories for Alterdot and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dashpay/alterdot
...
in the Alterdot repository under 'Fetch and create inputs' to install sources which require
...
To build Alterdot (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Alterdot repository.
...
```bash
    ...
    From https://github.com/dashpay/alterdot
...
For example:
```bash
URL=https://github.com/crowning-/alterdot.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit alterdot=${COMMIT} --url alterdot=${URL} ../dash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alterdot=${COMMIT} --url alterdot=${URL} ../dash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alterdot=${COMMIT} --url alterdot=${URL} ../dash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the alterdot git repository with the desired tag must both be available locally, and then gbuild must be
...
BTCPATH=/some/root/path/alterdot
SIGPATH=/some/root/path/alterdot-detached-sigs

./bin/gbuild --url alterdot=${BTCPATH},signature=${SIGPATH} ../dash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alterdot-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alterdot-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/alterdot-osx-build.assert
```
