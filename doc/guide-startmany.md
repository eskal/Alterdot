...
### Send 1000 BCRS to New Addresses

Send exactly 1000 BCRS to each new address created above.
...
Copy the masternode private key and corresponding collateral output transaction that holds the 1000 BCRS.

*Note: The masternode private key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 BCRS on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the bitcreds github repo](https://github.com/dashpay/dash/blob/master/doc/masternode_conf.md)
...
## Update bitcreds.conf on server

If you generated a new masternode private key, you will need to update the remote `bitcreds.conf` files.

Shut down the daemon and then edit the file.

```nano .bitcreds/bitcreds.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `bitcreds.conf` file.
...
```bitcreds-cli getinfo```

and comparing with the official explorer at https://explorer.bitcreds.org/chain/Bitcreds
...
```
bitcreds-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<1000 BCRS address>",
...
Search your Masternodes on https://bitcredsninja.pl/masternodes.html
