...
bool CSuperblockManager::GetSuperblockPayments(int nBlockHeight, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    for (int i = 0; i < pSuperblock->CountPayments(); i++) {
        ...
        if (pSuperblock->GetPayment(i, payment)) {
            ...
            // TODO: PRINT NICE N.N ADOT OUTPUT
...
CAmount CSuperblock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    // some part of all blocks issued during the cycle goes to superblock, see GetBlockSubsidy ADOT unused for Alterdot, just placeholder
...
void CSuperblock::ParsePaymentSchedule(const std::string& strPaymentAddresses, const std::string& strPaymentAmounts)
{
    ...
    */
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        if (!address.IsValid()) {
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid Alterdot Address : " << vecParsed1[i];
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
        /*
