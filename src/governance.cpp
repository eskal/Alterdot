    ...
    while (it != mapObjects.end()) {
        ...
        if ((pObj->IsSetCachedDelete() || pObj->IsSetExpired()) &&
            ...
            if (pObj->GetObjectType() == GOVERNANCE_OBJECT_PROPOSAL) {
                ...
                int64_t nSuperblockCycleSeconds = Params().GetConsensus().nSuperblockCycle * Params().GetConsensus().nPowTargetSpacing; // TODO_BCRS not really a necessary change as we don't use superblocks, make sure it can't break anything
...
bool CGovernanceManager::MasternodeRateCheck(const CGovernanceObject& govobj, bool fUpdateFailStatus, bool fForce, bool& fRateCheckBypassed)
{
    ...
    int64_t nTimestamp = govobj.GetCreationTime();
    int64_t nNow = GetAdjustedTime();
    int64_t nSuperblockCycleSeconds = Params().GetConsensus().nSuperblockCycle * Params().GetConsensus().nPowTargetSpacing; // TODO_BCRS
