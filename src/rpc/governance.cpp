...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 ADOT irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on alterdotcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in alterdot.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("alterdot.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in alterdot.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Alterdot features */
    { "alterdot",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "alterdot",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "alterdot",               "gobject",                &gobject,                true,  {} },
    { "alterdot",               "voteraw",                &voteraw,                true,  {} },
